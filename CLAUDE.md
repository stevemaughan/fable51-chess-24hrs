...
- Engine name (reported by `id name`): **`Fable 5.1 chess 24hrs`**
- Author (reported by `id author`): **`Fable 5.1`**
- Executable name: `Fable51chess24hrs.exe` (no spaces, so it's easy to pass to tools)
...
final/                    the final executable (Fable51chess24hrs.exe) — nothing else required
...
```
resources\fastchess\fastchess.exe --compliance final\Fable51chess24hrs.exe
```
...
```
resources\fastchess\fastchess.exe ^
  -engine cmd=final\Fable51chess24hrs.exe name=mine ^
...
- **What this is** — the benchmark: one model (Fable 5.1), 24 hours, from
...
large temp files (the deliverable `final/Fable51chess24hrs.exe` *is* tracked, so
